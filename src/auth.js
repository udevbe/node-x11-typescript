...
function readXauthority(cb, options) {
  var filename = options && options.xAuthority ? options.xAuthority : process.env.XAUTHORITY || path.join(homedir(), '.Xauthority')
  fs.readFile(filename, function(err, data) {
    if (!err)
      return cb(null, data)
    if (err.code == 'ENOENT') {
      // Xming/windows uses %HOME%/Xauthority ( .Xauthority with no dot ) - try with this name
      filename = options ? options.xAuthority : process.env.XAUTHORITY || path.join(homedir(), 'Xauthority')
      fs.readFile(filename, function(err, data) {
        if (err.code == 'ENOENT') {
          cb(null, null)
        } else {
...
module.exports = function(display, host, socketFamily, cb, options) {
  var family
